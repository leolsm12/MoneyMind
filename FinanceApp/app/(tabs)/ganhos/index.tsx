...
export default function GanhosScreen() {
  ...
  const [ganhos, setGanhos] = useState<{ id: number; valor: string; categoria: string; data: Date }[]>([]);
  ...
  const handleSaveGanho = async (data: Record<string, any>) => {
    ...
    try {
      const response = await axios.post(`${API_URL}/transacoes`, novoGanho, config);
      setGanhos(prev => [
        {
          id: response.data.id,
  ...
  return (
    <ThemedView style={styles.container}>
      ...
      >
        <AntDesign name="plus-circle" size={60} color="#EE6C4D" />
      </TouchableOpacity>
